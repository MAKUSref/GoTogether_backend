...
const readProfileRooms = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const userId = req.headers[X_USER_ID];
    const rooms = yield roomManager.readProfileRooms(userId);
    return res.status(200).json(Object.assign({}, rooms));
});
exports.readProfileRooms = readProfileRooms;
const readRoomByPin = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const pin = req.params.pin;
    const room = yield roomManager.readRoomByPin(Number(pin));
    if (room !== undefined && room !== null) {
        return res.status(200).json({ room });
    }
    return res.status(404).json({ room, message: "No rooms were found." });
...
const acceptRequest = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { roomId, userId } = req.body;
    const requestingUserId = req.headers[X_USER_ID];
    const isUserAccepted = yield roomManager.acceptRequest(roomId, userId, requestingUserId);
    if (isUserAccepted) {
...
const grantHost = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { roomId, userId } = req.body;
    const requestingUserId = req.headers[X_USER_ID];
    const isUserAccepted = yield roomManager.grantHost(roomId, userId, requestingUserId);
    if (isUserAccepted) {
        return res.status(201).send({ message: "User accepted." });
    }
    return res.status(400).send({ message: "Something went wrong!" });
